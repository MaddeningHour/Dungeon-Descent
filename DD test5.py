...
class characterType(ABC):
    ...
    def openInventory(self):
        ...
        myChoice = input('What item would you like to use?(Press enter exit inventory without losing your turn)')
        
        
        if myChoice == ' ':
            return False
        else:
            self.health += self.inventory[().heal]
...
def MultipleEnemiesIsAlive(enemies):
    for enemy in enemies:
        if not isinstance(enemy, enemyType):
            raise ValueError(f'Could not validate {enemies[enemy]}')
        if enemy.is_alive():
            return True
        else:
...
def gameStart():
    ...
    myEnemies = MultipleEnemies(skeletonEnemy, skeletonEnemy2, batEnemy)
...
def interactionLogic3(Player, Enemies):
    runAway = False   
    ...
    while MultipleEnemiesIsAlive(Enemies):
        for currentEnemy in Enemies:
            if not currentEnemy.is_alive():
                continue
            myChoice = Player.PlayerActionChoices(currentEnemy)
...
def test():
    ...
    myEnemies = MultipleEnemies(skeletonEnemy, skeletonEnemy2, batEnemy)
    print(MultipleEnemiesIsAlive(myEnemies))
gameStart()